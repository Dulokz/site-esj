...
export class GraphAPI {
  ...
  async validate(token, hints) {
    ...
    const phone = hints.phone_number_id ? numbers.find((number) => number.id === hints.phone_number_id) : numbers.length===1 ? numbers[0] : null;
  ...
  async verifyCoexistence(token, phoneNumberId) {
    const phone = await this.request(phoneNumberId, token, { fields: 'is_on_biz_app,platform_type' });
    if (phone?.is_on_biz_app !== true || phone?.platform_type !== 'CLOUD_API') throw new MetaError('coexistence_not_confirmed', 409);
    return { isOnBizApp: true, platformType: 'CLOUD_API' };
  }
