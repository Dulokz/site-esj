...
export function signupStore(db) {
  return {
    insert: async (item) => db.query(`INSERT INTO signup_attempts(id,tenant_id,user_id,session_id,state_hash,expires_at,outcome,signup_mode) VALUES($1,$2,$3,$4,$5,$6,'pending',$7)`, [randomUUID(), item.tenantId, item.userId, item.sessionId, item.stateHash, new Date(item.expiresAt), item.signupMode]),
    consume: async (item) => {
    ...
    consumeDetailed: async (item) => {
      const result = await db.query(`UPDATE signup_attempts SET consumed_at=now() WHERE state_hash=$1 AND tenant_id=$2 AND user_id=$3 AND session_id=$4 AND consumed_at IS NULL AND expires_at>now() RETURNING id,signup_mode`, [item.stateHash, item.tenantId, item.userId, item.sessionId]);
      if(result.rowCount===1)return {consumed:true,located:true,expired:false,used:false,contextMatch:true,reason:'consumed',signupMode:result.rows[0].signup_mode};
      const found=await db.query('SELECT tenant_id,user_id,session_id,consumed_at,expires_at,signup_mode FROM signup_attempts WHERE state_hash=$1 LIMIT 1',[item.stateHash]);
      ...
      return {consumed:false,located:true,expired,used,contextMatch,reason:!contextMatch?'context_mismatch':used?'already_used':expired?'expired':'not_consumed',signupMode:row.signup_mode};
    },
  };
}
