...
import { consumeSignupAttemptDetailed, isWhatsAppSignupMode, parseSignupResult } from '../meta/embedded-signup.js';
import { callbackPayloadStructure,metaCallbackDiagnostic,safeErrorCode } from './meta-diagnostics.js';

const publicConnection = (row) => ({
  id: row.id, wabaId: row.waba_id, phoneNumberId: row.phone_number_id, displayPhoneNumber: row.display_phone_number,
  status: row.status, signupMode: row.signup_mode, isOnBizApp: row.is_on_biz_app, platformType: row.platform_type,
  webhookMessagesReceived: Boolean(row.webhook_messages_received), webhookSmbMessageEchoReceived: Boolean(row.webhook_smb_message_echo_received),
  connectedAt: row.connected_at, validatedAt: row.validated_at,
});
...
export async function provision(db, context, id, graph) {
  ...
  try {
    ...
    const outcome=await transaction(async (tx) => {
      ...
      if (verified.phone.status !== 'CONNECTED' && row.signup_mode === 'cloud_api') {
...
export async function completeSignup({ body, db, context }) {
  ...
  try {
    ...
    const signupMode = stateStatus.signupMode;
    if (!isWhatsAppSignupMode(signupMode) || result.signup_mode !== signupMode) throw new HttpError(409, 'signup_mode_mismatch');
    ...
    const coexistence = signupMode === 'coexistence' ? await graph.verifyCoexistence(accessToken, verified.phone.id) : null;
    id = await transaction(async (tx) => {
      ...
      const reference = await vault.storeCredential({ tenantId: context.tenantId, provider: 'meta_whatsapp', secret: { token: accessToken, registrationPin: priorSecret?.registrationPin || String(randomInt(0, 1000000)).padStart(6,'0') } });
      const connectionId = old?.id || randomUUID();
      await tx.query(`INSERT INTO whatsapp_connections(id,tenant_id,meta_business_id,waba_id,phone_number_id,display_phone_number,status,credential_reference,validated_at,signup_mode,is_on_biz_app,platform_type)
        VALUES($1,$2,$3,$4,$5,$6,'pending',$7,now(),$8,$9,$10) ON CONFLICT(id) DO UPDATE SET meta_business_id=excluded.meta_business_id,waba_id=excluded.waba_id,
        display_phone_number=excluded.display_phone_number,status='pending',credential_reference=excluded.credential_reference,validated_at=now(),signup_mode=excluded.signup_mode,is_on_biz_app=excluded.is_on_biz_app,platform_type=excluded.platform_type,updated_at=now()`,
      [connectionId, context.tenantId, verified.businessId, verified.waba.id, verified.phone.id, verified.phone.display_phone_number, reference, signupMode, coexistence?.isOnBizApp ?? null, coexistence?.platformType ?? null]);
...
export const statusHandler = api('GET', async ({ db, context }) => {
  const { rows } = await db.query(`SELECT c.*,
    EXISTS(SELECT 1 FROM webhook_events e WHERE e.connection_id=c.id AND e.tenant_id=c.tenant_id AND e.kind='message') AS webhook_messages_received,
    EXISTS(SELECT 1 FROM webhook_events e WHERE e.connection_id=c.id AND e.tenant_id=c.tenant_id AND e.kind='smb_message_echo') AS webhook_smb_message_echo_received
    FROM whatsapp_connections c WHERE c.tenant_id=$1 ORDER BY c.created_at DESC`, [context.tenantId]);
  ...
  return { status, canConnect, tenantName: context.tenantName, connections: rows.map(publicConnection) };
});
...
export async function sendTestMessage({ db, context, body }) {
  ...
  if (!claimed) {
    const { rows } = await db.query('SELECT meta_message_id,created_at FROM message_requests WHERE id=$1 AND tenant_id=$2 AND connection_id=$3', [requestId,context.tenantId,id]);
    return { accepted: true, duplicate: true, messageId: rows[0]?.meta_message_id || null, acceptedAt: rows[0]?.created_at || null };
  }
  const graph = new GraphAPI(config);
  const result = await transaction(async (tx) => {
    await lockTenant(tx, context.tenantId);
    ...
    try {
      ...
      return { accepted: true, messageId: sent.messages[0].id, acceptedAt: new Date().toISOString() };
    } catch (error) {
...
export async function readMessageStatus({ db, context, body }) {
  objectBody(body, ['connectionId','requestId']);
  const connectionId = uuid(body.connectionId), requestId = uuid(body.requestId);
  const row = await connection(db, context, connectionId, ['connected','pending','error','reauthorization_required','not_connected']);
  const request = await db.query('SELECT meta_message_id,status,created_at FROM message_requests WHERE id=$1 AND tenant_id=$2 AND connection_id=$3', [requestId,context.tenantId,connectionId]);
  if (!request.rows[0]) throw new HttpError(404, 'message_not_found');
  const messageId = request.rows[0].meta_message_id;
  const delivery = messageId ? await db.query("SELECT delivery_status,received_at FROM webhook_events WHERE tenant_id=$1 AND connection_id=$2 AND kind='status' AND message_id=$3 ORDER BY received_at DESC LIMIT 1", [context.tenantId,row.id,messageId]) : { rows: [] };
  return { accepted: request.rows[0].status === 'sent', messageId: messageId || null, acceptedAt: request.rows[0].created_at, deliveryStatus: delivery.rows[0]?.delivery_status || null, deliveryUpdatedAt: delivery.rows[0]?.received_at || null };
}
export const messageStatusHandler = api('POST', readMessageStatus, {limit:20});
