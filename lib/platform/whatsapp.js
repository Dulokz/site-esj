...
import { consumeSignupAttempt, parseSignupResult } from '../meta/embedded-signup.js';
...
export async function completeSignup({ body, db, context }) {
  ...
  let result;
  try { result = parseSignupResult(body); } catch { throw new HttpError(400, 'invalid_parameters'); }
  if (!await consumeSignupAttempt(result.state, context, signupStore(db))) throw new HttpError(403, 'invalid_state');
  let id;
  try {
    if (result.error) throw new HttpError(400, 'meta_authorization_cancelled');
    if (!result.waba_id || !result.phone_number_id) throw new HttpError(400, 'missing_assets');
...
export const callbackHandler = api('POST', completeSignup, { limit: 10 });
