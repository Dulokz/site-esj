  ...
  try {
    ...
    await transaction(async (tx) => {
      for (const record of records) {
        const { rows } = await tx.query("SELECT id,tenant_id FROM whatsapp_connections WHERE waba_id=$1 AND phone_number_id=$2 AND status IN ('connected','pending')", [record.wabaId,record.phoneId]);
        if (!rows[0]) continue;
        await tx.query('INSERT INTO webhook_events(event_hash,tenant_id,connection_id,kind,delivery_status) VALUES($1,$2,$3,$4,$5) ON CONFLICT(event_hash) DO NOTHING', [record.eventHash,rows[0].tenant_id,rows[0].id,record.kind,record.status]);
      }
    },database());
